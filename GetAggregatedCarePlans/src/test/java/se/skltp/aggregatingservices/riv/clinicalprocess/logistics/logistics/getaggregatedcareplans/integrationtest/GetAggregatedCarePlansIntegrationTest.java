...
public class GetAggregatedCarePlansIntegrationTest extends AbstractAggregateIntegrationTest {

    @SuppressWarnings("unused")
    private static final Logger log = LoggerFactory.getLogger(GetAggregatedCarePlansIntegrationTest.class);
    ...
    private static final String DEFAULT_SERVICE_ADDRESS = GetAggregatedCarePlansMuleServer.getAddress("SERVICE_INBOUND_URL");

    protected String getConfigResources() {
        return "soitoolkit-mule-jms-connector-activemq-embedded.xml," + 
               "GetAggregatedCarePlans-common.xml," +
        //      Only load GetAggregatedCarePlans-common.xml, it will import the other config files since mule-deploy.properties can't load config-files
        //      from jar-files on the classpath, e.g. 
        //      agp-core.jar
    ...
    /**
     * Perform a test that is expected to return one hit with data from one
     * source system
     */
    @Test
    ...
    public void test_ok_many_hits_with_partial_timeout() {
        ...
        // Verify the Processing Status, expect ok from source system #1 and #2
        // but a timeout from #3
        assertProcessingStatusDataFromSource(statusList.get(0), TEST_LOGICAL_ADDRESS_1);
    ...
    private List<ProcessingStatusRecordType> doTest(String registeredResidentId, String senderId, String originalConsumerHsaId,
        ...
        GetAggregatedCarePlansTestConsumer consumer = new GetAggregatedCarePlansTestConsumer(DEFAULT_SERVICE_ADDRESS, senderId, originalConsumerHsaId);
        ...
        GetCarePlansResponseType response = responseHolder.value;
        int expextedResponseSize = testData.length;

        assertEquals(expextedResponseSize, response.getCarePlan().size());
        ...
        // Verify the size of the processing status and return it for further
        // analysis
        ...
        // Verify that correct "x-vp-sender-id" http header was passed to the
        // engagement index
        assertEquals(SKLTP_HSA_ID, EngagemangsindexTestProducerLogger.getLastSenderId());

        // Verify that correct "x-rivta-original-serviceconsumer-hsaid" http
        // header was passed to the engagement index
        assertEquals(SAMPLE_ORIGINAL_CONSUMER_HSAID, EngagemangsindexTestProducerLogger.getLastOriginalConsumer());

        // Verify that correct "x-vp-sender-id" and "x-rivta-original-serviceconsumer-hsaid" http headers were passed to the service producer,
